...
const focusColour = (focus: string) => {
  if (focus.includes("Lead") || focus.includes("research")) return "rgba(79,70,229,0.10)";
  if (focus.includes("Cold") || focus.includes("email")) return "rgba(2,132,199,0.10)";
  ...
  if (focus.includes("sales")) return "rgba(220,38,38,0.10)";
  return "rgba(43,63,124,0.06)";
};

const focusDot = (focus: string) => {
  if (focus.includes("Lead") || focus.includes("research")) return "#4f46e5";
  if (focus.includes("Cold") || focus.includes("email")) return "#0284c7";
  if (focus.includes("Call") || focus.includes("follow")) return "#0d9488";
  if (focus.includes("CRM") || focus.includes("warm")) return "#ea580c";
  if (focus.includes("sales")) return "#dc2626";
  return "var(--text-3)";
};
    ...
    <div className="page">
      <style>{`
        ...
        .roster-card:hover {
          ...
          box-shadow: 0 8px 28px rgba(23,42,94,0.14);
        }
        .roster-day-row { transition: background 0.2s ease; }
        .roster-day-row:hover { background: rgba(43,63,124,0.04); }
        ...
        .roster-del:hover { color: var(--danger); background: rgba(220,38,38,0.10); }
      ...
      <div className="page-header roster-anim">
        <div>
          <h1 style={{
            ...
            background: "linear-gradient(90deg, var(--text-1) 30%, #4f46e5 80%, #0284c7 100%)",
        ...
        {DAYS.map((day, i) => {
          ...
          return (
            <div key={day} className="roster-day-row" style={{
              ...
              background: isToday ? "rgba(79,70,229,0.05)" : "transparent",
            }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", width: 120, flexShrink: 0 }}>
                {isToday && (
                  <span className="roster-today-dot" style={{
                    width: 7, height: 7, borderRadius: "50%",
                    background: "#4f46e5", flexShrink: 0, display: "inline-block",
                  }} />
                )}
                <span style={{ fontWeight: isToday ? 800 : 600, color: isToday ? "#4f46e5" : "var(--text-1)", fontSize: "0.9rem" }}>
                  {day}
                </span>
              </div>
                  ...
                  {rows.map((row) => (
                    <div key={row.id} style={{
                      ...
                      <span style={{
                        ...
                        background: row.hours !== null ? "rgba(79,70,229,0.10)" : "rgba(43,63,124,0.06)",
                        color: row.hours !== null ? "#4f46e5" : "var(--text-3)",
                      }}>

...
function followUpStatus(p: Potential): "overdue" | "due_today" | "upcoming" | "none" {
  if (!p.follow_up_date) {
    // Auto-calculate: 5 days after updated_at for contacted/qualified stages
    if (!["contacted","qualified","proposal"].includes(p.status)) return "none";
  ...
  const today = new Date(); today.setHours(0,0,0,0);
  const due = new Date(p.follow_up_date); due.setHours(0,0,0,0);
                        ...
                        <div key={p.id} style={{
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginTop: "0.2rem" }}>
                              ...
                              {p.assigned_to && <span style={{ fontSize: "0.7rem", color: "#4f46e5" }}>👤 {p.assigned_to}</span>}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ padding: "0.75rem" }}>
                          ...
                          {p.assigned_to && (
                            <div style={{ display: "flex", alignItems: "center", gap: "0.35rem", fontSize: "0.72rem", color: "#4f46e5", marginBottom: "0.25rem" }}>
                          ...
                          {(() => {
                            ...
                            return (
                              <div style={{ display: "inline-flex", alignItems: "center", gap: "0.25rem", fontSize: "0.66rem", fontWeight: 700, color: col, background: `${col}18`, border: `1px solid ${col}30`, borderRadius: 6, padding: "0.18rem 0.45rem", marginBottom: "0.25rem" }}>
                                <CalendarClock size={10} /> {followUpLabel(p)}
                              </div>
                            );
                          })()}
                          {p.notes && (
                      ...
                      <div key={p.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem", background: "var(--surface)", border: `1px solid ${isSelected ? "rgba(124,133,243,0.45)" : onCall ? "rgba(34,211,238,0.2)" : "var(--border)"}`, borderRadius: 12, padding: "0.8rem 1rem", marginBottom: "0.4rem" }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", marginTop: "0.2rem" }}>
                            ...
                            {p.assigned_to && <span style={{ display: "flex", alignItems: "center", gap: "0.25rem", fontSize: "0.75rem", color: "#4f46e5" }}><UserCircle2 size={10} />{p.assigned_to}</span>}
          ...
          <div className="modal">
            ...
            <div style={{ display: "grid", gap: "1rem" }}>
              {([["business_name", "Business Name", true], ["contact_name", "Contact Name", false], ["phone", "Phone", false], ["email", "Email", false]] as const).map(([key, label, required]) => (
                <div key={key}>
                  <label style={{ display: "block", fontSize: "0.75rem", fontWeight: 600, color: "var(--text-2)", marginBottom: "0.35rem" }}>
                    {label}{required && <span style={{ color: "#4f46e5", marginLeft: 2 }}>*</span>}
                  </label>
              ...
              <div>
                <label style={{ display: "block", fontSize: "0.75rem", fontWeight: 600, color: "var(--text-2)", marginBottom: "0.35rem" }}>
                  Follow-up Date
                  <span style={{ marginLeft: "0.4rem", fontSize: "0.68rem", color: "var(--text-3)", fontWeight: 500 }}>(defaults to 5 days after last update)</span>
                </label>
                <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                  ...
                  <button
                    type="button"
                    onClick={() => { const d = new Date(); d.setDate(d.getDate() + 5); setForm({ ...form, follow_up_date: d.toISOString().slice(0,10) }); }}
                    style={{ padding: "0.6rem 0.8rem", background: "var(--surface-3)", border: "1px solid var(--border-2)", borderRadius: 9, fontSize: "0.78rem", fontWeight: 600, color: "var(--text-2)", cursor: "pointer", whiteSpace: "nowrap" }}
                  >+5 days</button>
                  {form.follow_up_date && (
                    <button
                      type="button"
                      onClick={() => setForm({ ...form, follow_up_date: "" })}
                      style={{ padding: "0.6rem 0.6rem", background: "none", border: "1px solid var(--border)", borderRadius: 9, cursor: "pointer", color: "var(--text-3)", display: "flex" }}
                    ><X size={13} /></button>
                  )}
                </div>
              </div>
            ...
            <div style={{ display: "flex", gap: "0.75rem", marginTop: "1.5rem" }}>
              <button onClick={save} className="btn-primary" style={{ flex: 1, justifyContent: "center", background: "linear-gradient(135deg, #4f46e5, #6366f1)" }}>

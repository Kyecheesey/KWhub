...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const prev = await sql`SELECT status, assigned_to FROM activities WHERE id = ${id}`;
  ...
  if (assigned_to && old && old.assigned_to !== assigned_to) {
    await notifyAssignment({ kind: "activity", title, assignee: assigned_to, dueDate: due_date, priority, description });
  }
